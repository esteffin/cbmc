...
class smt_incremental_dry_run_solver : public smt_base_solver_processt
{
public:
  /// \param message_handler:
  ///   The messaging system to be used for logging purposes.
  ...
  smt_incremental_dry_run_solver(
    message_handlert &message_handler,
  ...
  ~smt_incremental_dry_run_solver() override = default;

protected:
  /// Pointer to the file stream to print the SMT formula. `nullptr` if output is
  ///    to `std::cout`.
  std::unique_ptr<std::ostream> file_stream;
